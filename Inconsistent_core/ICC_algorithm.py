...
#this version ignores lines that are commented out (begin with //)
#and includes an option to remove entire experiments at a time (not just lines)
...
### DIDNT ACTUALLY IMPLEMENT OPTION YET###
def findCore(file, option, eNames, n=0, i=0):
    ...
    outputFile = f"/media/sf_artifact/examples/temp/newfile{i}.rein"
    fr = open(file, "r")
...
def findExperimentNames(file) -> set():
    ...
    while line: 
        if "#Experiment" in line: 
            start = line.find("#Experiment")
            end = line.find("[")
            e = line[start:end]
            print(e, start, end, line)
            experiments.add(e.upper())
        line = f.readline()
...
def main():
    file = sys.argv[1]
    if len(sys.argv) > 2: option =int(sys.argv[2])
    else: option = 1
